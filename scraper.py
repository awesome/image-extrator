...
class MediaScraper(Scraper):
    ...
    def __init__(self, url):
        ...
        if not self.video_id:
            ...
            logging.debug('reverting to regular scraper: %s' % url)
...
def youtube_in_google(google_url):
    ...
    try:
        ...
        logging.debug('%s is really %s' % (google_url, youtube_url))
        return youtube_url
...
class GootubeScraper(MediaScraper):
    ...
    def largest_image_url(self):
        ...
        if m:
            ...
            image_url = safe_eval_str(image_url)
            return image_url
...
class OEmbed(Scraper):
    ...
    def download(self):
        ...
        if not self.content:
            ...
            logging.warn('oEmbed call (%s) failed to return content for %s'
                    %(api_url, self.url))
            return None
        ...
        try:
            ...
            logging.error('oEmbed call (%s) return invalid json for %s' 
                      %(api_url, self.url))
            return None
...
class EmbedlyOEmbed(OEmbed):
    ...
    domains = ['23hq.com', '5min.com', '99dollarmusicvideos.com',
        ...
        'saynow.com', 'schooltube.com', 'sciencestage.com', 'scrapblogging.com',
    ...
    def find_media_object(self, scraper):
        ...
        if scraper.soup:
            ...
            if movie_embed:
                ...
                logging.debug('found youtube embed %s' % youtube_url)
